...
//Them san pham
function AddProduct($id,$name,$price,$quantity,$decription,$status,$avatar){
    global $db;    
    $like = 100;
	$stmt = $db->prepare("INSERT INTO `products` (`Id`,`Name`,`Like`,`Price`,`StoreQuantity`,`Decription`,`Status`) VALUES(?,?,?,?,?,?,?)");
    $stmt->execute (array($id,$name,$like,$price,$quantity,$decription,$status));
    $len = count($avatar);
    $i2 = 0;
while($i2 < $len){
